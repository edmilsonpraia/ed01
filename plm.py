...
def get_binary_file_downloader_html(file_content, file_name):
...
def show_course_content(course_selection):
    ...
    if selected_lesson in course['lessons']:
        ...
        if 'video' in lesson_content:
            video_content = base64.b64decode(lesson_content['video']['content'])
            st.video(video_content)
        if 'pdf' in lesson_content:
            pdf_content = base64.b64decode(lesson_content['pdf']['content'])
            st.markdown(get_binary_file_downloader_html(pdf_content, lesson_content['pdf']['details']['FileName']), unsafe_allow_html=True)
